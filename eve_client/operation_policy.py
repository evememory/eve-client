...
def validate_action_policy(action: PlannedAction, config: ResolvedConfig) -> None:
    if action.action_type == "auth_setup":
        if action.path is not None or action.scope != "state":
            raise OperationPolicyError(
                f"{action.action_id} auth_setup must be state-scoped with no path"
            )
        return
    ...
    if action.action_type in {"write_config", "write_hooks_config"}:
        ...
        if action.scope != expected_scope or not _is_allowed_config_path(
            ...
            raise OperationPolicyError(
                f"{action.tool} config path not allowed by policy: {resolved_path}"
            )
        return

    if action.action_type == "create_companion_file":
        if action.tool == "claude-code":
            is_allowed_global = action.scope == "global-config" and _matches_path_suffix(
                resolved_path, (".claude", "CLAUDE.md")
            )
            is_allowed_project = action.scope == "project" and resolved_path == (
                config.project_root / "CLAUDE.md"
            ).resolve(strict=False)
            if not (is_allowed_global or is_allowed_project):
                raise OperationPolicyError(
                    f"{action.tool} companion path not allowed by policy: {resolved_path}"
                )
            return
        if action.tool == "gemini-cli":
            is_allowed_global = action.scope == "global-config" and _matches_path_suffix(
                resolved_path, (".gemini", "GEMINI.md")
            )
            is_allowed_project = action.scope == "project" and resolved_path == (
                config.project_root / "GEMINI.md"
            ).resolve(strict=False)
            if not (is_allowed_global or is_allowed_project):
                raise OperationPolicyError(
                    f"{action.tool} companion path not allowed by policy: {resolved_path}"
