...
def test_validate_hermes_profile_rejects_missing_or_malformed_values(profile: str | None) -> None:
...
def test_connect_rejects_older_hermes_before_profile_inspection(monkeypatch) -> None:
    ...
    monkeypatch.setattr("eve_client.hermes.shutil.which", lambda name: "/usr/local/bin/hermes")
...
def test_connect_reauthenticates_matching_existing_entry(monkeypatch) -> None:
    run = Mock(
        side_effect=[
            ...
            _completed([], stdout='{"url": "https://mcp.evemem.com/mcp", "auth": "oauth"}'),
    ...
    assert connect_hermes_profile("team_1", "https://mcp.evemem.com/mcp") == "reauthenticated"
...
def test_connect_rejects_conflicting_entry_without_mutation(monkeypatch) -> None:
    run = Mock(
        side_effect=[
            _version(),
            _completed(["hermes", "profile", "show", "team_1"]),
            _completed([], stdout='{"url": "https://other.example/mcp", "auth": "oauth"}'),
        ]
    )
    monkeypatch.setattr("eve_client.hermes.shutil.which", lambda name: "/usr/local/bin/hermes")
    monkeypatch.setattr("eve_client.hermes.subprocess.run", run)
    ...
    assert [call.args[0] for call in run.call_args_list] == [
        ...
        ["hermes", "--profile", "team_1", "config", "get", "mcp_servers.eve-memory", "--json"],
    ]
...
def test_verify_runs_profile_scoped_test(monkeypatch) -> None:
    run = Mock(side_effect=[
        _version(),
        _completed(["hermes", "profile", "show", "team_1"]),
        _completed([], stdout='{"url": "https://mcp.evemem.com/mcp", "auth": "oauth", "enabled": true}'),
        _completed([]),
    ])
    monkeypatch.setattr("eve_client.hermes.shutil.which", lambda name: "/usr/local/bin/hermes")
    monkeypatch.setattr("eve_client.hermes.subprocess.run", run)
    ...
    assert run.call_args_list[3].args[0] == [
        "hermes", "--profile", "team_1", "mcp", "test", "eve-memory"
    ]
...
def test_connect_rejects_entry_disabled_after_login(monkeypatch) -> None:
    run = Mock(side_effect=[
        ...
        _completed([], stdout='{"url": "https://mcp.evemem.com/mcp", "auth": "oauth"}'),
        _completed([]),
        _completed([], stdout='{"url": "https://mcp.evemem.com/mcp", "auth": "oauth", "enabled": false}'),
    ])
    monkeypatch.setattr("eve_client.hermes.shutil.which", lambda name: "/usr/local/bin/hermes")
    monkeypatch.setattr("eve_client.hermes.subprocess.run", run)
